...
def FedKDMRV11(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        if iter >= args.first_stage_bound:
            ...
            for i, idx in enumerate(idxs_users):
                ...
                w = local.train(net=net_S,net_T=net_T,KD_alpha=args.KD_alpha)
        ...
        if iter >= args.first_stage_bound:
            ...
            w_locals_KD = recombination(w_locals, m)
...
def FedKDMRV12(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
...
def Fed2WKDMRV11(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        if iter >= args.first_stage_bound:
            for i, idx in enumerate(idxs_users):
                ...
                local = LocalUpdate_KD(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w = local.train(net=net_L,net_T=net_G,KD_alpha=args.KD_alpha)
                w_locals[i] = copy.deepcopy(w)
                net_L.load_state_dict(w_locals[i])
                L_G = LocalUpdate_KD(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w_g = L_G.train(net=net_G,net_T=net_L,KD_alpha=args.KD_alpha)
        ...
        if iter >= args.first_stage_bound:
            ...
            w_locals_KD = recombination(w_locals_KD, m)
            for i in range(len(w_locals)):
                w_locals[i] = copy.deepcopy(w_glob)
        else:
            for i in range(len(w_locals)):
...
def Fed2WKDMRV12(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
