...
def Fed2Codl(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        # update global weights
        if iter >= args.first_stage_bound:
            selected_w_locals_G = [w_locals_KD[i] for i in idxs_users]  # only get weights from selected clients
            w_glob = Aggregation(selected_w_locals_G, None)
        else:
            selected_w_locals = [w_locals[i] for i in idxs_users]  # only get weights from selected clients
            w_glob = Aggregation(selected_w_locals, None) # Global Model Generation
