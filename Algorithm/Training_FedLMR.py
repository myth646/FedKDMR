...
def Layerwise_recombination(w_locals, m):

    w_locals_new = copy.deepcopy(w_locals)

    layer_to_models = defaultdict(list)
    for model_idx, model in enumerate(w_locals):
        for layer_name in model.keys():
            layer_to_models[layer_name].append(model_idx)
    
    for layer_name, available_models in layer_to_models.items():
        random.shuffle(available_models)
        for i in range(m):
            if layer_name in w_locals_new[i]:
                donor_idx = available_models[i % len(available_models)]
                w_locals_new[i][layer_name] = copy.deepcopy(w_locals[donor_idx][layer_name])
...
def FedKDMRV3(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        if iter >= args.first_stage_bound:
            KD_alpha = get_dynamic_KD_alpha(iter, args.first_stage_bound, args.KD_buffer_bound, args.KD_alpha, args.Dynamic_KD_power)
            for i, idx in enumerate(idxs_users):
                ...
                w = local.train(net=net_S,net_T=net_T,KD_alpha=KD_alpha)
...
def Fed2WKDMRV3(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        if iter >= args.first_stage_bound:
            ...
            for i, idx in enumerate(idxs_users):
                ...
                local = LocalUpdate_KD_d(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w = local.train(net=net_L,net_T=net_G,KD_alpha=KD_alpha)
                w_locals[i] = copy.deepcopy(w)
                net_L.load_state_dict(w_locals[i])
                L_G = LocalUpdate_KD_d(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w_g = L_G.train(net=net_G,net_T=net_L,KD_alpha=KD_alpha)
...
def get_dynamic_KD_alpha(iter , first_stage_bound, KD_buffer_bound , max_alpha=0.3, power=1):
    bound = first_stage_bound + KD_buffer_bound
    if iter >= bound or first_stage_bound == 0:
        return max_alpha
    else:
        # iter 从0线性增加到 first_stage_bound，alpha 从0增加到 max_alpha
        progress = ((iter - first_stage_bound ) / KD_buffer_bound) ** power
