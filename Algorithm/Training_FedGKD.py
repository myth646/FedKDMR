    ...
    def train(self, net, net_T):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if self.args.GKD_balance: 
                    combined_loss = (1 - self.args.KD_alpha) * predictive_loss + self.args.KD_alpha * distillation_loss
                else:
                    combined_loss =  predictive_loss + self.args.KD_alpha * distillation_loss
                loss= combined_loss
...
def FedGKD(args, net_glob, dataset_train, dataset_test, dict_users):
    net_glob.train()
    ...
    for iter in range(args.epochs):
        ...
        if iter >= args.first_stage_bound:
            ...
            for idx in idxs_users:
                ...
                net_T.load_state_dict(avg_ensemble_model)
        ...
        selected_w_locals = [w_locals[i] for i in idxs_users]  # only get weights from selected clients
        w_glob = Aggregation(selected_w_locals, None) # Global Model Generation

        # copy weight to net_glob
        net_glob.load_state_dict(w_glob)
        history_global_models.append(copy.deepcopy(w_glob))
        
        
        if len(history_global_models) > args.history:
            history_global_models.pop(0)  
        if len(history_global_models) > 0 and iter >= args.first_stage_bound -1:
            avg_ensemble_model = calculate_ensemble_average(history_global_models)
...
def calculate_ensemble_average(model_state_dicts):
    ...
    keys = model_state_dicts[0].keys()
    
    for key in keys:
        sum_tensor = None
        
        for model_state in model_state_dicts:
            if sum_tensor is None:
                sum_tensor = model_state[key].clone().float()
            else:
                sum_tensor += model_state[key].float()
        
        averaged_state_dict[key] = sum_tensor / len(model_state_dicts)
